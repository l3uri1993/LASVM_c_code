...
int count_svs()
{
	...
	for(i=0;i<alpha.used;i++) 	// Count svs..
	{
...
void finish(lasvm_t *sv)
{
	...
	//for(i=0;i<m;i++)
		//alpha.array[i]=0;
	floatResizeArray(&alpha,1);
	alpha.array[0] = 0;
	float svalpha[l];
	lasvm_get_alpha(sv,svalpha);
	for(i=0;i<svs;i++)
		floatInsertArray(&alpha,svalpha[i]);
